...
##############################################################
def DoPhot(model=0, renuc=1, **Config):
    ...
    IMAGE_IN    = "masked_se" + suffix # "resid" + suffix   ## can be resid + (extended objects + Dmask --> dophot)
...
def read_dpo_file(dpo_file, Xctr, Yctr, ZP=0):
    ...
    df = pd.DataFrame(dpo_data[:,:10], columns=
                        [
                            "id",
                            "star",
                            "xpos",
    ...
    df["cxx"] = (np.cos(df.tilt)**2 / df.AA**2) + (np.sin(df.tilt)**2 / df.BB**2)
    df["cyy"] = (np.sin(df.tilt)**2 / df.AA**2) + (np.cos(df.tilt)**2 / df.BB**2)
    df["cxy"] = 2.*np.cos(df.tilt)*np.sin(df.tilt) * (1./df.AA**2 - 1./df.BB**2)

    # Additional correction at the faint end based on Cho et al. (2016) comparison
    df["magautocor"] = df.apply(lambda row: row.magauto-0.038*(row.magauto-24) if row.magauto>24 else row.magauto, axis=1)
...
##############################################################
def make_do_lkn(dpo_file, model=0, r_aperture=0, exclude457=False, ZP=0, **Config):
    ...
    with open(lkn_file_name, 'w') as alk:
        ...
        for i in range(len(df)):
            ...
            alk.write('%5s %8.2f %7.2f %8.2f  1 %9.5f %8.5f %9.5f  %5s %7.4f %5s %8.4f %6.4f %6.3f %6.4f\n' \
                    %(R.id, R.xpos, R.ypos, R.radius, R.cxx, R.cyy, R.cxy, 
                    R.kron, R.AA, R.id, R.mtot, R.mtoterr, R.sestar, R.star))
        ...
        for i in range(len(df)):
            ...
            if R.radius > 20:
                if R.isgal == 1:
                    ...
                    ds9_file.write("circle(%.4f, %.4f, 3) # color=red \n"%(R.xpos+0.5,R.ypos+0.5))
