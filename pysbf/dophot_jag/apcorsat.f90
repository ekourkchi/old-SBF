...
subroutine apcorsat(big,noise,nfast,nslow,starpar,itype,npstar,nsmax,&
  ...
  integer :: itype(nsmax)
  real(8) :: starpar(npstar,nsmax),appar(npap,nsmax),dummy(npstar)
  real(4) :: big(nfast,nslow),noise(nfast,nslow)
  real(4) :: xy(2),rrmag(2)
  real(8) :: rmag(2,nsmax),apcmag(nsmax),ee(nsmax)
  real(8),allocatable :: apparmagn(:),dummagn(:)
!  real(8) :: apparmagn(2),dummagn(2)
  character(*) :: flags(nff)
  external onestar
  external surface1
...
!print*,apval,skyval,aparea,skywt,skyval/skywt
     ...
     iap=iap+1
!JT FIXME: NOTE THIS IS VERY STRANGE.
     apcmag(iap) =  (2.5*dlog10(ratlum)) - appar (3,i)
     apcmag(iap) =  (2.5*dlog10(ratlum))
