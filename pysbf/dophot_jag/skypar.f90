...
subroutine skypar(nstot,nfast,nslow,maxfil,nsmax,npstar,&
  ...
  real(8) :: skyplanepar(npskyp),skyhubpar(npskyh),skyhubparinit(npskyh)
  real(8) :: skymedarray(nfast/iskymedstep,nslow/iskymedstep)
  real(4) :: big(nfast,nslow),noise(nfast,nslow)
  real(8) :: starpar(npstar,nsmax),shadow(npstar,nsmax),ava(npstar)
  real(8) :: xy(2,maxfil),z(maxfil),ze(maxfil)
  real(8) :: accskyp(npskyp),alimskyp(npskyp),accskyh(npskyh),alimskyh(npskyh)
  real(8) :: dummyskyp(npskyp,npskyp),dummyskyh(npskyh,npskyh)
  real(8) :: values((nfast*nslow)/(iskymedstep*iskymedstep))
  integer :: itype(nsmax)
  character(*) :: flags(nff)
...
!JT: previous was     iskyskip = 1
!JT: keep the number of sky samples to 25<N<81
     iskyskip = max(1, iskymedstep / 5)
     jm = 0
     do j=iskymedstep,nslow,iskymedstep
        ...
        do i=iskymedstep,nfast,iskymedstep
           ...
           nvalues = 0
!JT: don't need to median *every* point!
           do jj=jjlo,jjhi,iskyskip
              do ii=iilo,iihi,iskyskip
                 if (noise(ii,jj).ge.rmagic) cycle
                 nvalues = nvalues + 1
                 values(nvalues) = big(ii,jj)
              end do
           end do
