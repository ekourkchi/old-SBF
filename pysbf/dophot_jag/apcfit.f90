...
subroutine apcfit(x,y,mag,apin,apout,nap,files,nff,niter,lverb,&
  ...
  real(8) :: x(nap),y(nap),mag(nap),apin(nap),apout(nap)
  real(8) :: xy(2,nap),rmag(2,nap)
  real(8) :: xyref(2,1),rmagref(2,1)
  real(8) :: apparmag(npapmag),dumarrayf(npapmag)
  real(8) :: apparxy(npapxy),dumarray(npapxy)
  real(8) :: apcxy(nap),apcmag(nap),ee(nap)
  real(8) :: resid(nap),apcfitt(nap),apcbig(nap)
  integer :: indexx(nap)
  character(*) :: files(nff)
  external surface
  external surface1
...
subroutine fixcoeff(onestar,vals,apcvals,ee,nap,lverb,nit,npap,coeffs)
  ...
  IMPLICIT REAL(8) (A-H,O-Z)
  real(8) :: vals(2,nap),apcvals(nap),ee(nap)
  real(8) :: resid(nap)
  real(8) :: coeffs(npap),err(npap)
  real(8) :: wacc(npap), wlim(npap)
  external onestar
...
subroutine find_sigma(onestar,vals,apcvals,ee,nap,coeffs,npap,resid,sigmaval)
  ...
  IMPLICIT REAL(8) (A-H,O-Z)
  real(8) :: vals(2,nap),apcvals(nap),ee(nap),resid(nap)
  real(8) :: coeffs(npap),dumarray(npap)
  external onestar
...
subroutine getapcbig(apcbig,ee,nap,apcb,apcbsig)
  ...
  IMPLICIT REAL(8) (A-H,O-Z)
  real(8) :: apcbig(nap),ee(nap),aphold(nap)
...
subroutine write_report(file,nap,indexx,ee,x,y,mag,resid,apcfitt,apcbig,&
  ...
  IMPLICIT REAL(8) (A-H,O-Z)
  character(*) :: file
  real(8) :: apparxy(npapxy),apparmag(npapmag)
  integer indexx(nap)
  real(8) ee(nap),x(nap),y(nap),mag(nap),resid(nap),apcfitt(nap),apcbig(nap)
