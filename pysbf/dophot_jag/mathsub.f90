...
function value(xx, yy, gauss, psf, npsf, dvdx, dvdy)
  ...
  real(8) :: psf(maxpsf, maxpsf), gauss(5)
...
!JT THIS surely is a bug!
!JT  rinter = cubint(g(0),dy,dfdy)
!JT  dfdx = cubint(dgdx(0),dy,dummy)
  rinter = cubint(g,dy,dfdy)
  dfdx = cubint(dgdx,dy,dummy)
  return 
...
function erff(xin, xo, beta, dfdxo, dfdbet)
  ...
  x = ((xin - xo) / beta) ** 2
  f = exp(- (0.5 * x))
  n = max(2,int((3.247*((f*abs((x*(x-6.))+3.))**0.25))/beta)+1)
...
!   Is CUT1 above or below the minimum currently-accepted value?
     ...
     redo = .false.
     istep = sign(1.0001d0,cut1 - sky(minimum + 1))
...
!   Is CUT2 above or below the current maximum?
     ...
     istep = sign(1.0001d0,cut2 - sky(maximum))
