...
subroutine apcorr2(big,noise,nfast,nslow,starpar,itype,npstar,nsmax,&
  ...
  integer :: itype(nsmax)
  real(8) :: starpar(npstar,nsmax),appar(npap,nsmax)
  real(4) :: big(nfast,nslow),noise(nfast,nslow)
  real(8) :: apcorr(napertures),apsum(napertures),aparea(napertures)
  real(8) :: apx(nstot),apy(nstot),apamag(nstot)
  real(8) :: apinner(nstot),apouter(nstot)
  real(8),allocatable :: apparxy(:),apparmag(:)
