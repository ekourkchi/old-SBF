...
subroutine twofit(starpar,a,npstar,twostar,par1,par2,&
  ...
  real(8) :: starpar(npstar),a(npstar)
  real(8) :: acc(npstar),alim(npstar),bacc(npstar+3),blim(npstar+3)
  real(8) :: a5(npsffit),a6(npsffit),a7(npsffit),ava(npstar)
  real(8) :: xy(2,maxfil),z(maxfil),ze(maxfil)
  real(8) :: b(npstar+3),berr(npstar+3),b1(npstar),b2(npstar)
  character(*) :: flags(nff)
...
! JT: I may have broken this by changing the sign of angle in ellip.f90
! but this is the correct sign if "x&y are positively correlated, ..."
