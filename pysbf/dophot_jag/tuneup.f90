...
!   18    image_var (y/n)              18   image_var

! JT add nsmin and nsfrac as a parameter
...
subroutine tuneup(npmax,nff,flags,files,skyguess,fitrad,maskrad,&
  ...
  real(8),parameter :: fwhmfudge=1.0, rad2deg=57.29578, sig2fw=2.35482
  real(8) :: sig(3),ava(npmax),nsfrac
  real(8) :: acc(npmax),accskyp(npmax),accskyh(npmax)
  real(8) :: alim(npmax),alimskyp(npmax),alimskyh(npmax)
...
! Is there an input variance file?  Null if none.
  call readitem(nlines,header,'IMAGE_VAR',nr,itype,files(18),line)
  flags(18)(1:3) = 'NO '
  if(nr.gt.0) then
     flags(18)(1:3) = 'YES'
  end if
...
! Matching files?
  ...
  call readitem(nlines,header,'NSMIN',nr,itype,item,line)
  read(item,*) nsmin
  call readitem(nlines,header,'NSFRAC',nr,itype,item,line)
  read(item,*) nsfrac
...
! Ask for aperture correction parameters  
  flags(12)(1:3) = 'NO '
  call readitem(nlines,header,'APCORRFILE',nr,itype,files(12),line)
  if(nr.gt.0) then
     flags(12)(1:3) = 'YES'
  end if
