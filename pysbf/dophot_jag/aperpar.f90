...
subroutine aperpar(big,noise,nfast,nslow,starpar,npstar,rmagic,&
  ...
  real(8) :: starpar(npstar), appar(npap)
  real(8) :: apcorr(napertures),apval(napertures),aparea(napertures)
  logical :: badpix,skip
...
!JT add aperture magnitude noise calculation
  objvar = 0.0
  skyvar = 0.0
...
!          Fraction of the pixel inside the aperture circle
        ...
        objvar = objvar + fractnstar*fractnstar*noise(jx,jy)
        ...
        skyvar = skyvar + fractnsky*fractnsky/noise(jx,jy)
     end do
  end do
...
!       between the sky and fit mags.
        ...
        if (ratlum.gt.0) appar(3) = 2.5*dlog10(ratlum)
        apcorr(ii) = 2.5*dlog10(ratlum)
     end if
  end do
  appar(5) = 1.086*sqrt(objvar+skyvar*(aparea(napertures)/skywt)**2)/appar(1)
