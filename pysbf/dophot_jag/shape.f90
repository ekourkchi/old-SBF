...
!$  use omp_lib
  ...
  real(8) :: a(npstar),b1(npstar),b2(npstar),dummy(npstar)
  real(8) :: ava(npstar),acc(npstar),alim(npstar),starparrms(npstar)
  real(8) :: chi(4),sig(3)
...
!$OMP PARALLEL DO&
!$OMP&   PRIVATE(IT,YMIN,YMAX,I,K,NEWSTAR, A, XY,Z,ZE,NPT,AREAFILL,AREATOT, ENUFFPTS,&
!$OMP&           GALCHI, CONVERGE, OFFP, CHISQSHAPPE, VERYBIG, FIXXY, B1,B2,STARCHI)
...
!$OMP END CRITICAL
                 itype(newstar) = 3             
                 do k = 1,npstar
                    starpar(k,newstar) = b2(k)
                    shadow(k,newstar) = b2(k)
                 end do
