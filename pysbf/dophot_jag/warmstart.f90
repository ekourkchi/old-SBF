...
subroutine warmstart(big,noise,nfast,nslow,flags,files,nff,&
  ...
  real(8) :: starpar(npstar,nsmax),appar(npstar,nsmax)
  real(8) :: shadow(npstar,nsmax),shaderr(npstar,nsmax)
  real(8) :: summ(npstar),ava(npstar)
  integer :: itype(nsmax)
  ...
  close(unit=3)
  if(flags(7)(1:1).eq.'Y') close(unit=7)
